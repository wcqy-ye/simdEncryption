...
void SHA256_HashMultipleBlocks_ARMV8(uint32_t *state, const uint32_t *data, size_t length, int order);
...
void SHA256_HashMultipleBlocks_ARMV8(uint32_t *state, const uint32_t *data, size_t length, int order) 
{
    ...
    //CRYPTOPP_ASSERT(length >= SHA256_BLOCK_SIZE);
...
void SHA256_Update(SHA256_CTX *ctx, const uint8_t *data, size_t length) {
    printf("len:%d\n",ctx->buffer_len);
    size_t block_length = SHA256_BLOCK_SIZE - ctx->buffer_len;
    size_t remaining = length;

    // 如果已有缓冲区有数据，则先填充缓冲区
    if (ctx->buffer_len > 0) {
        if (remaining >= block_length) {
            memcpy(ctx->buffer + ctx->buffer_len, data, block_length);
            SHA256_HashMultipleBlocks_ARMV8(ctx->state, (const uint32_t *)ctx->buffer, SHA256_BLOCK_SIZE, 0);
            ctx->buffer_len = 0;
            data += block_length;
    ...
    while (remaining >= SHA256_BLOCK_SIZE) {
        SHA256_HashMultipleBlocks_ARMV8(ctx->state, (const uint32_t *)data, SHA256_BLOCK_SIZE, 0);
    ...
    // 将剩余数据存入缓冲区
    if (remaining > 0) {
        memcpy(ctx->buffer, data, remaining);
        ctx->buffer_len = remaining;
    }

    // 更新数据位数
    ctx->num_bits += length * 8;
}


void SHA256_Final(SHA256_CTX *ctx, uint8_t hash[32]) {
    // 填充缓冲区以达到512位（64字节）块大小
    ctx->buffer[ctx->buffer_len++] = 0x80;

    // 如果缓冲区无法容纳长度，则填充一个新的块
    if (ctx->buffer_len > (SHA256_BLOCK_SIZE - 8)) {
        memset(ctx->buffer + ctx->buffer_len, 0, SHA256_BLOCK_SIZE - ctx->buffer_len);
        SHA256_HashMultipleBlocks_ARMV8(ctx->state, (const uint32_t *)ctx->buffer, SHA256_BLOCK_SIZE, 0);
        ctx->buffer_len = 0;
    }

    // 填充零以获得新块的最后8个字节存储数据长度
    memset(ctx->buffer + ctx->buffer_len, 0, SHA256_BLOCK_SIZE - ctx->buffer_len - 8);

    // 将位数存入缓冲区的最后8个字节（按大端序）
    ctx->num_bits = SwapEndian32(ctx->num_bits);
    memcpy(ctx->buffer + SHA256_BLOCK_SIZE - 8, &ctx->num_bits, 8);

    // 计算最后一个块的哈希值
SHA256_HashMultipleBlocks_ARMV8(ctx->state, (const uint32_t *)ctx->buffer, ctx->buffer_len / sizeof(uint32_t), 0);
