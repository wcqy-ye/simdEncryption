...
int main(int argc, char* argv[])
{
    ...
    size_t pos = 0;
    while (pos + 64 <= message_len) {
        sha256_process(state, (const uint8_t*)(message + pos), 64);
        pos += 64;
    }

    /* Process the last block and add padding if necessary */
    uint8_t last_block[64];
    memset(last_block, 0, sizeof(last_block));
    memcpy(last_block, message + pos, message_len - pos);
    last_block[message_len - pos] = 0x80;

    if (message_len - pos >= 56) {
        sha256_process(state, last_block, 64);
        memset(last_block, 0, sizeof(last_block));
    }

    uint64_t message_bits = message_len * 8;
    size_t last_block_pos = 64 - sizeof(message_bits);
    for (size_t i = 0; i < sizeof(message_bits); i++) {
        last_block[last_block_pos + i] = (message_bits >> (56 - i * 8)) & 0xFF;
    }
    sha256_process(state, last_block, 64);
